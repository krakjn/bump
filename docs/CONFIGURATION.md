...
`bump print` (visible alias `p`). Flags are stackable except `--only-*` and `--full`.
All variants emit output **without a trailing newline**. Bare `bump` (no
...
```text
Print composed version from BUMPFILE without newline
